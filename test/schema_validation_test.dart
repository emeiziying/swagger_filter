...
void main() {
  group('JSON Schema Validation Tests', () {
    ...
    test('schema defines all required properties', () {
      ...
      expect(
          (swaggerSource['properties'] as Map).containsKey('source'), isTrue);
      expect((swaggerSource['properties'] as Map).containsKey('include_paths'),
          isTrue);
      expect((swaggerSource['properties'] as Map).containsKey('exclude_paths'),
          isTrue);
      expect((swaggerSource['properties'] as Map).containsKey('include_tags'),
          isTrue);
      expect((swaggerSource['properties'] as Map).containsKey('exclude_tags'),
          isTrue);
    ...
    test('path validation pattern works correctly', () {
      ...
      final pathPattern = schema['definitions']['SwaggerSource']['properties']
          ['include_paths']['items']['pattern'];
